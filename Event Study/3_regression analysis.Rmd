---
title: "Regression Analysis for  5d EMBI_Residual"
output:
  html_document:
    df_print: paged
...
```{r}
fit5_1 <- lm(e_embi_5d ~size + res_gdp_lag, main_df)
summary(fit5_1)
```
```{r}
#Add interaction
fit5_2 <- lm(e_embi_5d ~size * res_gdp_lag, main_df)
summary(fit5_2)
```
Specification 6: previous year's reserves to M2, and this variable interacted with Size BSL
```{r}
fit6_1 <- lm(e_embi_5d ~size + res_m2_lag, main_df)
summary(fit6_1)
```
```{r}
# Add interaction
fit6_2 <- lm(e_embi_5d ~ size * res_m2_lag, main_df)
summary(fit6_2)
```
Specification 7: previous year's short-term debt due to GDP, and this variable interacted with Size BSL
```{r}
fit7_1 <- lm(e_embi_5d ~size + debt_gdp_lag, main_df)
summary(fit7_1)
```

```{r}
## Add Interaction
fit7_2 <- lm(e_embi_5d ~size * debt_gdp_lag, main_df)
summary(fit7_2)
```
Specification 8: previous year's short-term debt due to M2, and this variable interacted with Size BSL
```{r}
fit8_1 <- lm(e_embi_5d ~size + debt_m2_lag, main_df)
summary(fit8_1)
```
```{r}
# Add interaction
fit8_2 <- lm(e_embi_5d ~size *  debt_m2_lag, main_df)
summary(fit8_2)
```
Specification 9: dummy=1 if overlapping with a fund program , and this variable interacted with Size BSL
```{r}
fit9_1 <- lm(e_embi_5d ~size + program, main_df)
summary(fit9_1)
```
```{r}
# Add Interaction
fit9_2 <- lm(e_embi_5d ~ size * program, main_df)
summary(fit9_2)
```
Specification 10: average spread in the previous month, and this variable interacted with Size BSL
```{r}
fit10_1 <- lm(e_embi_5d ~size + e_embi_l_20d, main_df)
summary(fit10_1)
```
```{r}
# Add residuals
fit10_2 <- lm(e_embi_5d ~size * e_embi_l_20d, main_df)
summary(fit10_2)
```
